...
func _process(_delta: float) -> void:
	if dragging and target:
		match freedom:
			DRAG_FREEDOM.FREE:
				target.global_position = target.get_global_mouse_position() - (Vector2.ZERO + target.pivot_offset)
			DRAG_FREEDOM.X_LOCKED:
				target.global_position.x = target.get_global_mouse_position().x - (Vector2.ZERO.x + target.pivot_offset.x)
			DRAG_FREEDOM.Y_LOCKED:
				target.global_position.y = target.get_global_mouse_position().y - (Vector2.ZERO.y + target.pivot_offset.y)
			_:
				target.global_position = target.get_global_mouse_position() - (Vector2.ZERO + target.pivot_offset)
